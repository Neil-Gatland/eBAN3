...
public class OSSCustomerBANBean extends BANBean
{
    private String SQL;
    private String Global_Account_Manager="",Customer_Billing_Address_Lines[]={"","","","","","",""};
    private String Customer_Contact="please Tel:  +44 (0)1908 833164 or fax +44 (0)1908 833152",Region_Code="";
    private String Business_Unit="",Customer_Billing_Name="",Customer_Contact_Name="";
    private String Customer_Billing_Address="";
    private Hashtable ScreenData=new Hashtable();
    protected Vector mandatory=new Vector();

  public OSSCustomerBANBean ()
  {
    mandatory.clear();
    ...
    mandatory.addElement("Account_Id");
  ...
  public void Reset()
  {
    ...
    Customer_Contact="please Tel:  +44 (0)1908 833164 or fax +44 (0)1908 833152";
    Region_Code="";
    Business_Unit="";
  ...
  public void setBusiness_Unit(String value)
  {
   Business_Unit = SU.isNull(value,"");
  }
  public void setRegion_Code(String value)
  {
   Region_Code = SU.isNull(value,"");
  }
...
/*get Methods*/
  ...
  {
     if ((FieldName.compareTo("Account_Id") ==0) ||
        (FieldName.compareTo("Global_Customer_Name") ==0))
    {
...
public boolean isValid(String ButtonPressed)
{
	...
	if ((Mode.compareTo("Add")==0) && (!Account_Id.startsWith("09")))
	{
	  setErrored("Account_Id");
	  Message = "<font color=red><b>The CCD Account number for OSS Accounts must start with '09'";
	  return false;
	}
	//Account already exists for a 'Create'?
	if ((Mode.compareTo("Add")==0) && (AccountExists()))
	{
	  setErrored("Account_Id");
	  Message = "<font color=red><b>An Account with this Account Id already exists";
	  return false;
	}
...
/*******************************************************************************/
 public boolean getCustomerBAN()
 {
    ...
    try{
      ...
      {
        if (groupName.equals("MANS"))
        {
          SQL = "{call eban..get_MANS_Customer_BAN ";
        }
        else
        {
          SQL = "{call eban..get_OSS_Customer_BAN ";
        }
        SQL += "(?)}";
	...
	try
	{
	  if (RS.next())
	  {
	    ...
	    Business_Unit=RS.getString(10);
	    Customer_Billing_Name=RS.getString(11);
	    Customer_Contact_Name=RS.getString(12);
	    Customer_Billing_Address_Lines[0]=RS.getString(13);
	    Customer_Billing_Address_Lines[1]=RS.getString(14);
	    Customer_Billing_Address_Lines[2]=RS.getString(15);
	    Customer_Billing_Address_Lines[3]=RS.getString(16);
	    Customer_Billing_Address_Lines[4]=RS.getString(17);
	    Customer_Billing_Address_Lines[5]=RS.getString(18);
	    Customer_Billing_Address_Lines[6]=RS.getString(19);
	    rejectReason=RS.getString(20);
	    Account_Id=RS.getString(21);
	    setCurrency_Desc(RS.getString(22));
	    Mode=(RS.getString(23));
            getCustomerBAN = true;
	  }
  ...
  int rowcount=0,i=0;
    ...
    try{
	  if (DBA.Connect(WRITE,P5))
	  {
            if (groupName.equals("MANS"))
            {
              SQL = "{call eban..Create_MANS_Customer_BAN ";
            }
            else
            {
	      SQL = "{call eban..Create_OSS_Customer_BAN ";
            }
	    SQL += "(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
  ...
  public boolean updateCustomerBAN()
  {
    ...
    try{
      if (DBA.Connect(WRITE,P5))
      {
        if (groupName.equals("MANS"))
        {
          SQL = "{call eban..update_MANS_Customer_BAN ";
        }
        else
        {
          SQL = "{call eban..update_OSS_Customer_BAN ";
        }
        SQL += "(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
	...
	cstmt.setString(8,Invoice_Region );
	cstmt.setString(9,Customer_Contact );
	cstmt.setString(10,Business_Unit );
	cstmt.setString(11,Customer_Billing_Name );
	cstmt.setString(12,Customer_Contact_Name );
	cstmt.setString(13,Customer_Billing_Address_Lines[0] );
	cstmt.setString(14,Customer_Billing_Address_Lines[1] );
	cstmt.setString(15,Customer_Billing_Address_Lines[2]);
	cstmt.setString(16,Customer_Billing_Address_Lines[3] );
	cstmt.setString(17,Customer_Billing_Address_Lines[4] );
	cstmt.setString(18,Customer_Billing_Address_Lines[5] );
	cstmt.setString(19,Customer_Billing_Address_Lines[6] );
	cstmt.setString(20,userId);
	cstmt.setString(21,rejectReason);
	cstmt.setString(22,Account_Id);
	cstmt.setString(23,Currency_Desc);
...
public boolean AuthoriseCustomerBAN()
{
    ...
    try{
      if (DBA.Connect(WRITE,P3))
      {
        if (Mode.compareTo("Create") ==0)
        {
	  SQL = "{call givn..Create_OSS_Customer ";
	  SQL += "(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
	...
	{
	  SQL = "{call givn_ref..Create_Account ";
	  SQL += "(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
	...
	{
	  SQL = "{call givn_ref..Update_Account ";
	  SQL += "(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
	  ...
	  cstmt.setString(17, Invoice_Region);
	}
        try
	{
	  cstmt.execute();
	  ...
	  DBA.Connect(WRITE,P5);

	  SQL = "{call eban..Archive_BAN ";
	  SQL += "(?,?)}";

	  cstmt = DBA.Conn.prepareCall(SQL);

	  cstmt.setString(1,banIdentifier);
	  cstmt.setString(2,"Customer Account");
	  int res=cstmt.executeUpdate();

	  if (res!=1)
	  {
	    Message="Warning-Your BAN was implemented, but could not be archived";
	    AuthoriseCustomerBAN = false;
	  }
          else
          {
...
/*  private void setCustomerHeader()
  {
    ...
    else
    {
      ...
      imagename1="sort_by.gif";
        ...
        String[] head ={"BAN Id","Status","BAN Id","Invoice Region","Global Srv Reference","Required Date","BAN Type","Credit/Debit","Description"};
	header="<table border="+border+"><tr class=gridHeader>";
	header+="<td class=gridHeader NOWRAP width=110>BAN Id</td><td class=gridHeader NOWRAP width=75>Status<br>&nbsp<image name=GSR onclick=\"sendOrderBy(2)\" align=right width=13 height=8 src=\"../nr/cw/newimages/"+imagename1+"\"> </td>";
	header+="<td class=gridHeader NOWRAP width=120>Invoice Region</td><td class=gridHeader NOWRAP width=125>Global Service<br>Reference<image onclick=\"sendOrderBy(4)\" name=GSR valign=bottom align=right width=13 height=8 src=\"../nr/cw/newimages/"+imagename2+"\"></td>";
	header+="<td class=gridHeader NOWRAP width=80>Required<br>Date<td class=gridHeader NOWRAP width=90>BAN Type</td>";
	header+="<td class=gridHeader NOWRAP width=90>Charge Type<td class=gridHeader NOWRAP width=52>Credit/<br>Debit</td>";
	header+="<td class=gridHeader NOWRAP width=105>Description</td><td class=gridHeader NOWRAP width=45>Select</td><td width=10 bgcolor=#FFFFFF>&nbsp</td></tr></table>";
	columns=9;
}*/
/*************************************************************************************/
...
/*************************************************************************************/
 public boolean getCustomer()
 {
    ...
    try{
      ...
      {
        SQL = "{call givn..get_OSS_Customer ";
        SQL += "(?)}";
	cstmt = DBA.Conn.prepareCall(SQL);
